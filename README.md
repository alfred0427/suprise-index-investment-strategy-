...
本研究主要發現如下：
- 1.驚奇效應持續
無論線性回歸或隨機森林模型，驚奇指數（Surprise Index）下一季股價均具顯著解釋力，其中「獲利相關指標」（EPS、毛利率、稅前 ROA）最為關鍵。
...
### 驚奇指數計算
本研究定義 **驚奇指數 (Surprise Index, SI)** 為公司財報表現與市場預期之落差，計算公式如下：

$$
SI_{i,t} = Z(\Delta FI^{YoY}_{i,t}) - Z(PR_{i,t-1})
$$

其中：  

- $\Delta FI^{YoY}_{i,t}$ = 財務指標的年增變化量 (Year-over-Year)  

<p align="center">
$\Delta FI^{YoY}_{i,t} = FI_{i,t} - FI_{i,t-4}$
</p>

- $PR_{i,t-1}$ = 上一季財報公布日後一日至本季財報公布日前一日的股價報酬率（先行反應）。  
- $Z(\cdot)$ = 標準化 (z-score)，在兩個變數相減前需先進行標準化處理。 

### 財務指標選取
為衡量公司獲利能力、營運效率及財務穩健性，本研究選取五項財務指標：  
1. 稅前 ROA (Return on Assets before tax)  
2. EPS (每股盈餘)  
3. 毛利率 (Gross Profit Margin)  
4. 營運現金流 (Operating Cash Flow)  
5. 流動比率 (Current Ratio)  

上述五項指標經公式轉換後，分別得到「驚奇 EPS、驚奇 ROA、驚奇毛利率、驚奇營運現金流、驚奇流動比率」，再共同構成綜合的 **驚奇指數**，作為後續實證分析與投資策略回測的基礎。
## 二、資料前處理
